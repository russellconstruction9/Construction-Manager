...
const Inventory: React.FC = () => {
    const { inventory, orderList, removeFromOrderList, clearOrderList, addManualItemToOrderList, currentUser } = useData();
        ...
        <div className="space-y-6">
            ...
            {inventory.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {inventory.map(item => (
                        <InventoryItemCard key={item.id} item={item} onEdit={isAdmin ? setEditingItem : () => {}} />
                    ))}
                </div>
            ) : (
                <EmptyState
